...
#define Assertf(op, ...)                             \
  do {                                               \
    ...
    float Timer::stop(int batch, const char* prefix, bool print) {
        checkRuntime(cudaEventRecord((cudaEvent_t)stop_, (cudaStream_t)stream_));
        checkRuntime(cudaEventSynchronize((cudaEvent_t)stop_));
    ...
    static void destroy_nvidia_pointer(_T* ptr) {
        if (ptr) delete ptr;  // 在TensorRT 10.3中使用delete替代destroy()
    }
    ...
    public:
        ...
        unordered_map<string, int> tensor_name_to_index_;
        ...
        void setup() {
            ...
            for (int i = 0; i < nbIOTensors; ++i) {
                ...
                tensor_name_to_index_[tensorName] = i;
            }
        }

        virtual int index(const std::string& name) override {
            auto iter = tensor_name_to_index_.find(name);
            Assertf(iter != tensor_name_to_index_.end(), "Can not found the tensor name: %s",
                name.c_str());
            return iter->second;
        }
        ...
        virtual bool is_input(int ibinding) override {
            auto engine = this->context_->engine_;
            const char* tensorName = engine->getIOTensorName(ibinding);
            return engine->getTensorIOMode(tensorName) == TensorIOMode::kINPUT;
        }
        ...
        virtual void print() override {
            ...
            for (int i = 0; i < engine->getNbIOTensors(); ++i) {
                const char* tensorName = engine->getIOTensorName(i);
                if (engine->getTensorIOMode(tensorName) == TensorIOMode::kINPUT)
                    num_input++;
                else
                    num_output++;
            }
            ...
            for (int i = 0; i < engine->getNbIOTensors(); ++i) {
                ...
                if (engine->getTensorIOMode(tensorName) == TensorIOMode::kINPUT) {
                    auto dim = engine->getTensorShape(tensorName);
                    INFO("\t%d.%s : shape {%s}", input_idx++, tensorName, format_shape(dim).c_str());
                }
            }
            ...
            for (int i = 0; i < engine->getNbIOTensors(); ++i) {
                const char* tensorName = engine->getIOTensorName(i);
                if (engine->getTensorIOMode(tensorName) == TensorIOMode::kOUTPUT) {
                    auto dim = engine->getTensorShape(tensorName);
                    INFO("\t%d.%s : shape {%s}", output_idx++, tensorName, format_shape(dim).c_str());
                }
            }
        }
    };
